...
cartContactRouter.post("/", async (req, res, next) => {
  try {
    const data = req.body;
...
cartContactRouter.put("/:cid", async (req, res, next) => {
  try {
    const cartContactId = req.params.cid;
    const newData = req.body;

    // Actualizar el cart contact utilizando el método update(cid, data) del CartContactManager
    const updatedCartContact = await cartContactManager.update(cartContactId, newData);
    ...
    if (updatedCartContact) {
      ...
      throw new Error(`Failed to update cart contact with ID ${cartContactId}.`);
    }
  } catch (error) {
    // Manejar errores utilizando el errorHandler
    return next(error);
...
cartContactRouter.delete("/:cid", async (req, res, next) => {
  try {
    const cartContactId = req.params.cid;

    // Eliminar el cart contact utilizando el método destroy(cid) del CartContactManager
    const deletedCartContact = await cartContactManager.destroy(cartContactId);
    ...
    if (deletedCartContact) {
      ...
      throw new Error(`Failed to delete cart contact with ID ${cartContactId}.`);
    }
  } catch (error) {
    // Manejar errores utilizando el errorHandler
    return next(error);
...
export default cartContactRouter;
