...
class UserRouter extends CustomRouter {
  init() {
    ...
    this.create("/",["USER"], validateUsersProps, async (req, res, next) => {
      try {
    ...
    this.update("/:uid",["USER"], validateUsersProps, async (req, res, next) => {
      try {
    ...
    this.destroy("/:uid",["USER"], async (req, res, next) => {
      try {
        const { uid } = req.params; // Obtener el ID del usuario de los parámetros de la URL
